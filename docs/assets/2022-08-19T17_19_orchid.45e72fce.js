...
`,e.exports.jsx(t.p,{children:"This carriage goes from any start_collection token until the next end_collection, collecting all tokens prefixed by an add command and ignoring the rest. The start and end tokens are consumed with the end token being replaced by the carriage's payload:"}),`
`,e.exports.jsx(r,{children:`a b start_collection c d add e add f g add add h end_collection i --r1
...
[] =1=> none`}),`
`,e.exports.jsx(t.p,{children:"I was going to include the ruleset for a match expression in this article too, but I realized that the specifics of how control may flow and types may be annotated across match arms depends heavily on the type system because ultimately the handlers need to be converted to N-ary lambda functions (N being the number of distinct placeholders) and the type checker needs to be able to statically assert from the match expression that N arguments will in fact be provided."}),`
`,e.exports.jsxs(t.p,{children:["I also wrote a small demo using the example of monads to demonstrate how lambda expressions are embedded in templates. For the purposes of this demo, ",e.exports.jsx(a,{c:"@T."})," indicates a generic over ",e.exports.jsx(a,{c:"T"}),", and ",e.exports.jsx(a,{c:"@:T U V."})," indicates a generic constraint for the existence of the relation or trait ",e.exports.jsx(a,{c:"T"})," for parameters ",e.exports.jsx(a,{c:"U"})," and ",e.exports.jsx(a,{c:"V"}),". For a rough explanation of how these work consult ",e.exports.jsx(t.a,{href:"https://github.com/lbfalvy/orchid#auto-parameters-generics-polymorphism",children:"the project readme"}),". Minor differences in semantics are to be expected, the type system is still in early development."]}),`
`,e.exports.jsx(r,{children:`-------- In main --------
...
export ::(static_map)`}),`
`,e.exports.jsxs(t.p,{children:[e.exports.jsx(a,{c:"static_map"})," as used by the ",e.exports.jsx(a,{c:"f"})," shorthand is also a fine example of a carriage, and of a helper rule being invoked by another rule without the programmer's knowledge to automate some subtask. Rules can form ecosystems like this, providing hooks or delegating work."]}),`
`,e.exports.jsxs(t.p,{children:["An ant property of this system is that compile-time structures can leak into runtime code. This is very useful in general because it enables a lot of powerful patterns for optimizing runtime, value-universe code depending on calling context, but in some cases it can cause problems on the boundary that are difficult to trace, for example, if ",e.exports.jsx(a,{c:"static_map"})," is called on a runtime conslist it will be left in the source even though it doesn't (and depending on the capabilities of the type system possibly can't) have a runtime definition. This problem can be solved with very low priority error reporting rules. The following is just an example:"]}),`
`,e.exports.jsx(r,{children:`-------- In std::macro_error --------
...
`,e.exports.jsx(t.p,{children:"Now when static_map is accidentally left in the code after all other macros had been executed, it will be converted to an error which will then bubble up to the AST root. Debugging tools can recognize these and produce a human-readable error trace."})]})}function c(o={}){const{wrapper:t}=o.components||{};return t?e.exports.jsx(t,Object.assign({},o,{children:e.exports.jsx(n,o)})):n(o)}export{c as default};
