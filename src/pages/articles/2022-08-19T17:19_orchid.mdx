import LambdaHighlight, {H} from "../../components/LambdaHighlight"

### The story so far

Almost half a year ago
[I wrote an article](/blog/2022-02-27T21:25_lambda_calculus_for_practical_use) about a
...
[the language now called Orchid](https://github.com/lbfalvy/orchid), which I still hope to
somehow incorporate into my final year project.

Orchid is essentially Lambda calculus, with the addition of type annotations.
...
The key feature that makes Orchid useful and sets it apart somewhat is the macro system.
...
This macro language is based on generalized kerning, a well-known Turing-complete problem.
An excellent breakdown of the proof can be seen in [this video](https://youtu.be/8_npHZbe3qM).
...
This carriage goes from any start_collection token until the next end_collection, collecting
...
Of course, this carriage isn't particularly useful, and defining stuff like infix operators in
...
The 2 in these placeholders is their _growth priority_. This has nothing to do with the rules'
...
Notice that multiplication has lower priority than addition, because both operators are eager.
...
To demonstrate the utility of this system, here's a set of rules to transform a  classic array
...
I was going to include the ruleset for a match expression in this article too, but I realized
...
I also wrote a small demo using the example of monads to demonstrate how lambda expressions are
embedded in templates. For the purposes of this demo, <H c='@T.'/> indicates a generic over
<H c='T'/>, and <H c='@:T U V.'/> indicates a generic constraint for the existence of
the relation or trait <H c='T'/> for parameters <H c='U'/> and <H c='V'/>.
For a rough explanation of how these work consult
[the project readme](https://github.com/lbfalvy/orchid#auto-parameters-generics-polymorphism).
...
<H c='static_map'/> as used by the <H c='f'/> shorthand is also a fine example of a
...
An important property of this system is that compile-time structures can leak into runtime code.
...
problems on the boundary that are difficult to trace, for example, if <H c='static_map'/>
...
this reason, elaborate macro-debugging tools will eventually be necessary.